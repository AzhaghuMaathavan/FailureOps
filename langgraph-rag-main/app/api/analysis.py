    ...
    if not latest_analysis or not latest_analysis.signal_packet:
        ...
        signal_schemas = [
            SignalItemSchema(
                ...
                name=s.name,
                canonical_name=getattr(s, "canonical_name", s.name),
                category=s.category,
                ...
                previous_risk_score=getattr(s, "previous_risk_score", None),
                baseline_risk_score=getattr(s, "baseline_risk_score", None),
                risk_change_percent=getattr(s, "risk_change_percent", None),
                risk_trend=getattr(s, "risk_trend", None),
                scoring_method=getattr(s, "scoring_method", None),
                benchmark_target=getattr(s, "benchmark_target", None),
                benchmark_critical=getattr(s, "benchmark_critical", None),
                unit=getattr(s, "unit", None),
                baseline_value=getattr(s, "baseline_value", None),
                previous_value=getattr(s, "previous_value", None),
                current_value=getattr(s, "current_value", None),
                baseline_timestamp=getattr(s, "baseline_timestamp", None),
                previous_timestamp=getattr(s, "previous_timestamp", None),
                current_timestamp=getattr(s, "current_timestamp", None),
                baseline_to_current_change_percent=getattr(s, "baseline_to_current_change_percent", None),
                previous_to_current_change_percent=getattr(s, "previous_to_current_change_percent", None),
                metric_change_percent=getattr(s, "metric_change_percent", None),
                metric_trend=getattr(s, "metric_trend", None),
